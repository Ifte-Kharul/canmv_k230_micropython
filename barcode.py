...
try:
    ...
    while True:
        ...
        os.exitpoint()
        img = sensor.snapshot()
        barcode_results = img.find_barcodes()
        if barcode_results:
#            print("Barcode Detection Result:", barcode_results)
#            print("barcode found");

            for code in img.find_barcodes():
                barcode_number = code.payload() # Get the barcode number (payload)
                 # Print just the number
                if barcode_number not in my_list:
                    print("Barcode Number:", barcode_number)
                    my_list.add(barcode_number)
                    img.draw_rectangle([v for v in code.rect()], color=(0, 0, 255))
...
#        print(fps.fps())
        letn = len(my_list)
        if letn>prev_len:
            print("Numbers of Barcode Detected is: %s"%letn)
            prev_len = letn
