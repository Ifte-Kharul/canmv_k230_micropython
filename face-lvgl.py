...
def btn_clicked_event(event):
    ...
    close_btn.add_event(close_popup, lv.EVENT.CLICKED, None)

def run_face_recognition(event):
    try:
         os.execv("/usr/bin/python3", ["python3", "/sdcard/examples/05-AI-Demo/face_recognition.py"])
    except Exception as e:
        print(f"Error running face_recognition.py: {e}")
        error_popup = lv.obj(lv.scr_act())
        error_popup.set_size(200, 100)
        error_popup.align(lv.ALIGN.CENTER, 0, 0)
        error_label = lv.label(error_popup)
        error_label.set_text(f"Error: {e}")
        error_label.align(lv.ALIGN.CENTER, 0, 0)
        close_btn = lv.btn(error_popup)
        close_btn.align(lv.ALIGN.BOTTOM_MID, 0, -10)
        close_btn_label = lv.label(close_btn)
        close_btn_label.set_text("Close")

        def close_error_popup(event):
            error_popup.delete()
        close_btn.add_event(close_error_popup, lv.EVENT.CLICKED, None)
...
def user_gui_init():
    ...
    run_btn = lv.btn(lv.scr_act())
    run_btn.align(lv.ALIGN.CENTER, 0, lv.pct(40)) #place run button under popup button
    run_label = lv.label(run_btn)
    run_label.set_text('Run Face Rec.')
    run_btn.add_event(run_face_recognition, lv.EVENT.CLICKED, None)
