    ...
    def create(self,sensor=None,hmirror=None,vflip=None,fps=60):
        with ScopedTiming("init PipeLine",self.debug_mode > 0):
            ...
            if brd=="k230d_canmv_bpi_zero":
                ...
                self.sensor.reset()
                self.sensor1 = Sensor(id = 1,fps=30) 
                ...
                self.sensor = Sensor(id = 0,fps=fps) if sensor is None else sensor
                self.sensor.reset()
                self.sensor1 = Sensor(id = 1,fps=fps)
            ...
            self.sensor.set_framesize(w = self.display_size[0]//2, h = self.display_size[1])
            ...
            #for second camera
            self.sensor1.set_framesize(w = self.display_size[0]//2, h = self.display_size[1])
            self.sensor1.set_pixformat(PIXEL_FORMAT_YUV_SEMIPLANAR_420)
            # 通道2给到AI做算法处理，格式为RGB888
            self.sensor1.set_framesize(w = self.rgb888p_size[0], h = self.rgb888p_size[1], chn=CAM_CHN_ID_2)
            # set chn2 output format
            self.sensor1.set_pixformat(PIXEL_FORMAT_RGB_888_PLANAR, chn=CAM_CHN_ID_2)
            ...
            #For Camera2
            sensor_bind_info1 = self.sensor.bind_info(x = self.display_size[0]//2, y = 0, chn = CAM_CHN_ID_0)
